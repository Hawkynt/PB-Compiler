...
public sealed class LoopVersioningTests {
  ...
  public void Loop_GivenAffineSubscript_ThenEndpointProofVersionsIt() {
    // INTEGER arithmetic in PB promotes through SINGLE, so use LONG here to exercise the integer
    // affine IR shape O0306 actually proves: counter + invariant constant, without a lossy FP round-trip.
    var fn = Lowered("""
      $ERROR BOUNDS ON
      SUB Work(BYVAL n&)
        DIM a%(0 TO 31), i&
