...
public sealed class O0291HandleOwnershipElisionTests {
  ...
  /// <summary>
  /// The assignment shape as the lowering really emits it, and the two conditions that make an
  /// ownership layer survive as far as O0291.
  ///
  /// <para>
  /// The statements live in a SUB because the MODULE BODY never releases its string locals - the
  /// process exit does - so a copy made there has no matching <c>rt_str_free</c> and is not a
  /// balanced pair at all. And the source is READ AGAIN afterwards, because
  /// <see cref="Mem2Reg.Run"/> runs O0296 <c>StringMove</c> first: an assignment that is the last
  /// use of its source is rewritten into a move, which removes the ownership layer before O0291
  /// could see it. What is left here is the case a move cannot have - both names live on.
  /// </para>
  /// </summary>
  [Test]
  public void Run_GivenTheCatalogueAssignmentShapeAfterLowering_WhenRun_ThenOneOwnershipLayerDisappears() {
    ...
    var body = module!.FindFunction("Catalogue")!;
    Mem2Reg.Run(body);
    var before = body.AllInstructions.OfType<IrCall>().Count(call => CallName(call) == "rt_str_dup");

    var changed = HandleOwnershipElision.Run(body);
    var after = body.AllInstructions.OfType<IrCall>().Count(call => CallName(call) == "rt_str_dup");

    Assert.Multiple(() => {
      Assert.That(changed, Is.GreaterThanOrEqualTo(1));
      Assert.That(after, Is.EqualTo(before - 1));
      Assert.That(IrVerifier.Verify(body), Is.Empty);
    });
  }
