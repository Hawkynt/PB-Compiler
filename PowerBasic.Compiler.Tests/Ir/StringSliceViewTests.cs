...
public sealed class StringSliceViewTests {
  ...
  public void SliceWithOpaqueArgumentCall_WhenOptimized_RemainsMaterialized() {
    var module = LowerOptimized("""
      ...
      FUNCTION Pick%(BYVAL value%) NOINLINE
        Pick% = value%
      END FUNCTION
      """, Dialect.Pb36);   // NOINLINE, which keeps the opaque call this test needs, is a 3.6 modifier
  ...
  private static IrModule? LowerOptimized(string source, Dialect dialect = Dialect.Pb35) {
