...
public sealed class CommandsWithNoEffectTests {
  ...
  private static (List<string> Errors, List<string> Warnings) Bind(string body, Dialect dialect = Dialect.Pb36) {
  ...
  private static readonly (string Keyword, string Statement)[] _spellings = [
    ...
    // It is bound as QuickBASIC: VIEW PRINT is Microsoft's, and no PowerBASIC dialect has it.
  ...
  public void Commands_GivenOneThatDoesNothing_WhenBound_ThenItIsAcceptedAndWarnedAbout((string Keyword, string Statement) form) {
    // VIEW PRINT is Microsoft's - PBC 3.0 and 3.5 answer it '"(" expected' - so it is asked of the
    // dialect that has it; every other entry is in both lineages.
    var (errors, warnings) = Bind(form.Statement, form.Keyword == "VIEW PRINT" ? Dialect.Qb45 : Dialect.Pb36);
    Assert.Multiple(() => {
