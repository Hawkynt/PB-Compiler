...
public sealed partial class InstructionSelector {
  ...
  private static (IrValue Source, short Magic, IrBinary HighShift, IrCast Truncate)? SignedMulHighShape(IrBinary multiply) {
    ...
    // The WIDENING may be shared, and in the programs this exists for it always is: two divisions of
    // the same dividend by different constants value-number to ONE sext, and O0056 lowers both. What
    // the idiom takes is the sext's 16-bit SOURCE, so another reader of the widened value neither
    // sees nor is seen by this IMUL - only the multiply itself must be single-use, because the fold
    // consumes its shift and truncate. Demanding a single-use sext instead declined every real
