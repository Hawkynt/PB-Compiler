...
public sealed partial class InstructionSelector {
    ...
    switch (cast.Op) {
      ...
      case IrCastOp.PtrToInt when to.IsInteger && to.Bits == 16: {
        ...
        if (cast.Value is IrFunction function) {
          ...
          var address = new MOperand.CodeOffset(function.Name);
  ...
  private bool SelectCall(IrCall call, MBlock block) {
    ...
    if (callee is not null)
      ...
      if (through is not (MOperand.Register or MOperand.Memory or MOperand.StackSlot
          or MOperand.DataCell or MOperand.ParamCell or MOperand.CodeOffset))
  ...
  private bool PushStackCallArgument(IrValue argument, string calleeName, out int bytes) {
    ...
    if (!this.TryOperand(argument, out var pushed))
      return false;
    if (pushed is MOperand.CodeOffset) {
  ...
  private bool TryOperand(IrValue value, out MOperand operand) {
    switch (value) {
      ...
      case IrFunction function:
        operand = new MOperand.CodeOffset(function.Name);
        return true;
