...
internal static class RuntimeAbi {
  ...
  private static readonly Dictionary<string, Routine> _routines = new(StringComparer.Ordinal) {
    ...
    // Microsoft Binary Format, the storage BASICA/GW floats live in. All four routines take the
    // cell's NEAR OFFSET rather than its bits: an MBF number is converted where it lies and never
    // becomes a register-resident value, so nothing else in this back end needs an MBF representation.
    ...
    ["rt_mbf64_load"] = new("rt_mbfld8", [new(ArgKind.Word, Reg.AX)], _callerSaved, Answer: ResultKind.St0),
    ["rt_mbf64_store"] = new("rt_mbfst8", [new(ArgKind.St0, default), new(ArgKind.Word, Reg.AX)], _callerSaved),
