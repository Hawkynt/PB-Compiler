...
/// proven safe, and a value with several readers may stay resident across basic-block edges when every
/// path admits the same x87 stack state. For adjacent pure x87 subtrees, O0348 also chooses the
/// lower-pressure evaluation order and inserts FXCH when a non-commutative root needs the original
/// operand order restored.
///
/// <para>
...
public static class X87StackOptimizer {
  ...
  private static bool TryProfileSubtree(MBlock block, int closingStore, out Subtree subtree) {
    ...
    for (var i = closingStore - 1; i >= 0; --i) {
      var instruction = block.Instructions[i];
      if (!CanReorder(instruction) || StackEffect(instruction) is not { } effect)
        return false;
      needed -= effect.Delta;
      if (needed < 0)
        return false;
  ...
  private static X87Effect? StackEffect(MInstr instruction) => instruction.Opcode switch {
    ...
    MOpcode.Fadd or MOpcode.Fsub or MOpcode.Fmul or MOpcode.Fdiv
      ...
      => new(1, 0),
    // FXCH swaps ST(0) with ST(1), so it needs two transient slots of its own; with only one it would
    // exchange a retained value out from under the region that owns it.
    MOpcode.Fxch => new(2, 0),
    _ => null,
  };
