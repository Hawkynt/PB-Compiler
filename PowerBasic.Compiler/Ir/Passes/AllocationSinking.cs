...
/// that body branches to the join, and the join immediately frees the owned handle. The other
/// successor is the join itself or an empty block that only branches to it - the lowering emits that
/// empty arm and SimplifyCfg has not necessarily removed it yet - so the untaken path executes no
/// work either way. Restricting the destination to one predecessor also keeps the sunk allocation
/// singly executed and avoids introducing a value that would fail to dominate a phi edge.
/// </para>
/// </summary>
public static class AllocationSinking {
  ...
  private static bool TrySink(IrBasicBlock source, IrCondBr branch, IrCall allocation) {
    ...
    if (!ReferenceEquals(SkipEmptyArm(other), merge) || !ReferenceEquals(free.Parent, merge))
      return false;
  ...
  /// <summary>
  /// The join a do-nothing arm leads to. A no-else IF lowers with an empty second arm holding one
  /// unconditional branch; following it through recognizes the shape before SimplifyCfg folds it,
  /// and an arm that does nothing but branch still executes no allocation.
  /// </summary>
  private static IrBasicBlock SkipEmptyArm(IrBasicBlock block)
    => block.Instructions is [IrBr forward] ? forward.Target : block;
