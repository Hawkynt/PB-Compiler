...
public static class ColdCodeOutlining {
  ...
  /// <summary>
  /// Puts an outlined helper back into every caller and deletes it; answers whether it did.
  ///
  /// <para>
  /// Outlining is worth doing only if the consumer can compile the result, and a private helper is one
  /// more whole function that has to select AND register-allocate. A consumer that cannot take it - the
  /// native x86 routing is the one that says so - would otherwise lose the entire CALLER too, which is
  /// a far worse trade than never having outlined at all. Undoing is exact rather than approximate
  /// because the extraction shape is fixed: the region has no live-outs, and <see cref="Outline"/>
  /// leaves the call site as the whole of its block - the call, and the return of its result - so
  /// putting it back is a branch into the cloned body. A call site in any other shape is refused
  /// outright rather than guessed at.
  /// </para>
  /// </summary>
  public static bool Reinline(IrModule module, IrFunction helper) {
    if (helper.IsDeclaration || !IsGeneratedHelper(helper))
      return false;

    var sites = module.Functions
      .Where(function => !function.IsDeclaration && !ReferenceEquals(function, helper))
      .SelectMany(function => function.AllInstructions)
      .OfType<IrCall>()
      .Where(call => ReferenceEquals(call.Callee, helper))
      .ToList();
    if (sites.Count == 0 || !sites.All(call => IsOutlinedCallSite(call, helper)))
      return false;
    ...
    for (var i = 0; i < sites.Count; ++i) {
      ...
      for (var parameter = 0; parameter < helper.Parameters.Count; ++parameter)
        seed[helper.Parameters[parameter]] = arguments[parameter];

      var cloned = IrCloner.Clone(block.Parent!, helper.Blocks, seed, $"recold{i}.");
      foreach (var instruction in block.Instructions.ToList())
        instruction.EraseFromParent();
      block.Append(new IrBr(cloned[helper.Blocks[0]]));
    }

    module.RemoveFunction(helper);
    return true;
  }

  /// <summary>The shape <see cref="Outline"/> leaves behind, and the only one <see cref="Reinline"/> undoes.</summary>
  private static bool IsOutlinedCallSite(IrCall call, IrFunction helper)
    => call.Parent is { Parent: not null } block
       && block.Instructions is [var first, IrRet returned]
       && ReferenceEquals(first, call)
       && (returned.Value is null || ReferenceEquals(returned.Value, call))
       && call.ArgCount == helper.Parameters.Count;
