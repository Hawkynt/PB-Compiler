...
public static class DeadParameterElimination {
  ...
  public static int Run(IrModule module) {
    ...
    // Both halves are ABI changes - one shrinks a signature, the other moves calls onto a private
    // clone - and only the direct calls in THIS module are rewritten to match. A consumer that can
    // still build a call from the source declaration therefore has to be able to turn them off.
  ...
  private static int EliminateDeadParameters(IrModule module) {
    ...
    for (var progress = true; progress;) {
      ...
      foreach (var function in module.Functions.ToList()) {
        ...
        function.SignatureRewritten = true;
  ...
  private static int CloneDominantLiteralShapes(IrModule module) {
    ...
    foreach (var function in module.Functions.ToList()) {
      ...
      // The clone is a private definition with no source procedure of its own, so the backend routes it
      // through the generated-definition bridge - which reads its frame off this signature and this
      // convention, and holds it to the rule that a clone is emitted only alongside the body it came
      // from. Both facts therefore have to travel with the function.
