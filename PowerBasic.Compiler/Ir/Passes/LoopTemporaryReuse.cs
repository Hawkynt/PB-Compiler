...
public static class LoopTemporaryReuse {
  ...
  private static bool TryReuse(CountedLoop loop) {
    ...
    // Keyed by identity: two structurally equal instructions are still two different positions in
    // the block, which is the whole point of an ordering index.
    var positions = new Dictionary<IrInstruction, int>(ReferenceEqualityComparer.Instance);
    for (var i = 0; i < body.Instructions.Count; ++i)
      positions[body.Instructions[i]] = i;
