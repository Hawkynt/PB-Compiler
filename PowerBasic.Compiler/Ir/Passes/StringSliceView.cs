...
public static class StringSliceView {
  ...
  private static SliceCandidate? ClassifySlice(IrValue value, IrInstruction consumer) {
    // Every slice entry below takes a base and at least one count, so a call with fewer arguments is
    // not one of them - and asking for its first argument is out of range rather than a mismatch.
    if (value is not IrCall { Callee: IrFunction sliceFn } slice || slice.ArgCount < 2
