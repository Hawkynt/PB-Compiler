...
/// included file's tokens in place, <c>$IF</c>/<c>$ELSE</c>/<c>$ENDIF</c> conditionally
/// drop regions based on named-constant (equate) values observed in the stream.
/// All other tokens - including equate definitions and other metastatements - pass through.
/// </summary>
public sealed class Preprocessor {
  ...
  private Preprocessor(ISourceProvider provider) => this._provider = provider;

  /// <summary>Expands <paramref name="mainFile"/> into a single token stream ending with one <see cref="TokenKind.EndOfFile"/>.</summary>
  public static IEnumerable<Token> Expand(string mainFile, ISourceProvider provider) {
    var self = new Preprocessor(provider);
  ...
  private IEnumerable<Token> ExpandFile(string text, string resolvedName) {
    ...
    try {
      using var tokens = Lexer.Tokenize(text, resolvedName).GetEnumerator();
      ...
      while (tokens.MoveNext()) {
        ...
        switch (token.Kind) {
          ...
          case TokenKind.MetaCommand when token.Text is "IF" or "ELSE" or "ENDIF":
            foreach (var t in this.FlushLine(pending))
              yield return t;
  ...
  private void HandleConditional(Token token, IEnumerator<Token> tokens) {
    switch (token.Text) {
      case "IF": {
        ...
        // skipping ended either at a live $ELSE (then $ENDIF is still owed) or at the $ENDIF itself
        if (SkipRegion(tokens, token, allowElse: true) == SkipStop.Else)
          this._openConditionals.Push(token);
        break;
      }
  ...
  private enum SkipStop { Else, Endif }

  /// <summary>Discards tokens until the matching <c>$ENDIF</c> (or live <c>$ELSE</c>), honoring nesting.</summary>
