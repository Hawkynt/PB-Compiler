...
| BASICA / GW-BASIC | `basica` `gw` | Microsoft (interpreters) | The classic line-numbered interpreters. Scalar SINGLE and DOUBLE cells use genuine 4/8-byte Microsoft Binary Format (MBF), with exact x87 conversion on load/store in both DOS emitters. Local-frame, array and procedure-ABI MBF coverage remains on the roadmap. |
