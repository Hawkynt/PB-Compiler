...
| `pb21` | `PB.EXE` 2.10 | `tests/diff/pb21/oracle.conf`: TP5-style IDE driven headlessly via AUTOTYPE (Esc, F3 load, F10 Compile→Destination→EXE file, Compile, quit) | **ACTIVE** - `--dialect pb21`; 2 batteries byte-identical; PB 3.5 semantics except VAL radix wraps to 16 bits like TB (VAL("&H10000") = 0) - integral FPU promotion, E+n exponents, 15-digit DOUBLE and six-zero fraction cutoff all already match 2.10 |
...
| `qb10` | `BASCOM.EXE` 1.00 + `LINK` | `tests/diff/qb10/oracle.conf`: `BASCOM T.BAS/O,T.OBJ;` + `LINK ...,BCOM10.LIB;` | **ACTIVE** - `--dialect qb10`; 2 batteries byte-identical; verified quirks: CINT/float-to-int rounds half AWAY from zero (CINT(2.5)=3, CINT(-2.5)=-3), sequential OUTPUT files end with a CP/M-style ^Z, otherwise the QB 4.x runtime model already holds in 1.0 (16-digit DOUBLE, D exponents, E+0n padding) |
| `qb30` | `QB.EXE` 3.00 + `LINK` | `tests/diff/qb30/oracle.conf`: `QB T.BAS/O,T.OBJ;` + `BCOM30.LIB` (QB.EXE doubles as the command-line compiler) | **ACTIVE** - `--dialect qb30`; the qb10 battery runs byte-identical (full BASCOM runtime heritage incl. ^Z and half-away rounding) |
