...
- **Microsoft Binary Format** (1 program) — BASICA/GW floats are stored in MBF, which the x87 cannot compute on, so every load converts to an x87 compute value and every store converts back. `rt_mbfld`/`rt_mbfst` handle MBF32; `rt_mbfld8`/`rt_mbfst8` handle MBF64 without narrowing its 56 significant bits through IEEE64. The routines take the cell's near OFFSET, so an MBF number is converted where it lies and never becomes a register-resident value. What originally blocked MBF32 was a level up: `mem2reg` had promoted the cell, and a phi has no address for the conversions to work through — so an MBF cell is refused promotion. Scalar frame cells and near static-array elements now keep that address through an alloca or computed element pointer. Dynamic/far-array elements and procedure ABI crossings remain separate address/ABI work and decline rather than lose their format.
