...
On the native x86-16 path the helper is a module-level function with no
`ProcedureSymbol`, and every part of that routing is keyed on one. It is routed
by `CodeGenerator.BackendGenerated.cs`, the same bridge
[O0283](O0283-context-sensitive-cloning.md) uses for its context clones: a
generated definition is selected, scheduled, allocated, labelled and emitted
like any other routed body, from its IR signature and definition ABI. The
extraction is also speculative there, which a clone is not - a helper the
register allocator cannot finish is put back into its caller by
`ColdCodeOutlining.Reinline`, because losing the helper would otherwise cost the
caller its native routing for no gain at all.
