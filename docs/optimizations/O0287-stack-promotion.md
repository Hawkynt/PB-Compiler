...
| **Stage** | Mid-end |
| **IR** | ✅ `Ir/Passes/StringStackPromotion.cs`, run by `CodeGenerator.Backend` after the string canonicalizers and before O0339 |
...
heap block, allocation or free is created.

That staging copy is what makes the finished shape x86-16 specific rather than
target-neutral, so the pass is not part of `IrPassManager.Standard()`. It runs
from `CodeGenerator.Backend` alongside the other steps that want the final,
target-shaped IR - after every string canonicalizer, and before
[O0339](O0339-memory-routine-by-size.md), which expands the two small
copies the same way it expands any other constant-size transfer. A hosted target
would pay for the staging copy and gain nothing, and the IR->BASIC writer has no
way to name a frame object, so neither sees it: both keep the handle graph.
