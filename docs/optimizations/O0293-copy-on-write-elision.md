...
- the source value and the duplicate each have at most one `rt_str_free` use;
- every other raw use of either value is another `rt_str_dup` borrow;
- all uses and any release are in the candidate's basic block;
...
A value the function never releases has a lifetime that ends after every local
one, so it counts as the later release. That is the ordinary shape for a
straight-line `main`, where the handle a string variable holds at `END` is never
handed back: after sharing, the one surviving block is a block the program already
leaked, so the earlier release is dropped and no new one is invented.
