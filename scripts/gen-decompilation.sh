...
# DECOMPILATION markers): for every pb36 feature/optimization example under
...
# splice the generated block into PB36.md between the markers
python3 - "$OUT" "$GEN" <<'PYEOF'
import sys
out, gen = sys.argv[1], sys.argv[2]
doc = open(out).read()
begin = "<!-- BEGIN GENERATED DECOMPILATION - scripts/gen-decompilation.sh rewrites everything between these markers -->"
end = "<!-- END GENERATED DECOMPILATION -->"
b, e = doc.index(begin) + len(begin), doc.index(end)
open(out, "w").write(doc[:b] + "\n\n" + open(gen).read().rstrip("\n") + "\n\n" + doc[e:])
PYEOF
rm -f "$GEN"

rm -rf "$TMP"
echo "updated $OUT ($(wc -l < "$OUT") lines, $(ls docs/decompilation/features/*.bas docs/decompilation/optimizations/*.bas 2>/dev/null | wc -l) examples)"
