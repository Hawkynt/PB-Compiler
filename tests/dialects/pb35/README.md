...
|---|---|---|---|
| [x] | Statement syntax and parameter combinations | 277 / 277 | all 277 accepted |
| [x] | Lowers to the IR | 206 / 277 | 206 of 277 reach the IR; the rest decline by name, which is the documented subset |
...
| [x] | Syntax belonging to another dialect is rejected | 26 / 26 | all 26 cleanly refused |
...
| [~] | Metastatements and their effect on the produced executable | 4 / 7 | 3 not reflected in the image: optimize.speed: '$OPTIMIZE SPEED' and '$OPTIMIZE SIZE' produce the SAME image, so the directive changed nothing - it should change the optimizer's objective, which changes which transformations it takes, asm.cpu.push-imm: '$CPU 8086' accepted it - PUSH with an immediate is 80286 and later; on an 8086 target it must be refused, asm.cpu.shift-imm: '$CPU 8086' accepted it - a shift by an immediate count above 1 is 80286 and later; the 8086 has only SHL r, CL |
