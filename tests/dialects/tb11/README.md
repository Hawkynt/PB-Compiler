...
|---|---|---|---|
| [x] | Statement syntax and parameter combinations | 258 / 258 | all 258 accepted |
| [x] | Lowers to the IR | 153 / 258 | 153 of 258 reach the IR; the rest decline by name, which is the documented subset |
...
| [x] | Syntax belonging to another dialect is rejected | 43 / 43 | all 43 cleanly refused |
...
| [~] | Metastatements and their effect on the produced executable | 3 / 5 | 2 not reflected in the image: cpu.tier: '$CPU 80286' and '$CPU 8086' produce the SAME image, so the directive changed nothing - it should change a wider instruction set for the same arithmetic - the 286's shift-by-immediate and PUSH imm, optimize.speed: '$OPTIMIZE SPEED' and '$OPTIMIZE SIZE' produce the SAME image, so the directive changed nothing - it should change the optimizer's objective, which changes which transformations it takes |
